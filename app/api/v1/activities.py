...
async def create_activity(activity: ActivityCreate, activity_service: ActivityService = Depends(), current_user: User = Depends(get_current_active_user)):
...
async def read_activity(activity_id: int, activity_service: ActivityService = Depends(), current_user: User = Depends(get_current_active_user)):
...
async def update_activity(activity_id: int, activity_update: ActivityUpdate, activity_service: ActivityService = Depends(), current_user: User = Depends(get_current_active_user)):
    activity = await activity_service.get_activity(activity_id)
    if activity is None:
        raise HTTPException(status_code=404, detail='Activity not found')
    if activity.user_id != current_user.id:
...
async def delete_activity(activity_id: int, activity_service: ActivityService = Depends(), current_user: User = Depends(get_current_active_user)):
    activity = await activity_service.get_activity(activity_id)
    if activity is None:
        raise HTTPException(status_code=404, detail='Activity not found')
    if activity.user_id != current_user.id:
...
async def calculate_health_score(days: int = 7, activity_service: ActivityService = Depends(), current_user: User = Depends(get_current_active_user)):
