...
class HealthScoreService:
    ...
    async def get_visible_health_score(
            self,
            score_id: int,
            user_id: int,
            is_admin: bool
        ...
        if is_admin:
            return await self.get_health_score(score_id)
    ...
    async def get_health_scores(self, user_id: int, skip: int = 0, limit: int = 100) -> List[HealthScore]:
        q = (
            ...
            .order_by(HealthScore.date.desc(), HealthScore.id.desc())
            .offset(skip)
            .limit(limit)
        )
    ...
    async def get_health_scores_by_date_range(
        ...
        q = (
            ...
            .where(
                and_(
                    HealthScore.user_id == user_id,
                    HealthScore.date >= start_date,  # inclusive
                    HealthScore.date <= end_date,  # inclusive
                )
            )
            .order_by(HealthScore.date.desc(), HealthScore.id.desc())
            .offset(skip)
